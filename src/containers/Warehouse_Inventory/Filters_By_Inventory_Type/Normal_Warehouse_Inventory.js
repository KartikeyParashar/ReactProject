...
class Normal_Warehouse_Inventory extends Component {
  state = {
    serialized_data: [],
    loading: true
  };
  ...
  async componentDidMount() {
    try {
      ...
      const res = await fetch('http://127.0.0.1:8080/wms/api/normal_warehouse_inventory_type/');
      const normal_warehouse_data = await res.json();
      this.setState({
        serialized_data: normal_warehouse_data["data"],
        loading: false
      });
    } catch (e) {
      console.log(e);
...
export default withRouter(Normal_Warehouse_Inventory);
