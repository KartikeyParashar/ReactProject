...
class Expired_Bin_Inventory extends Component {
  state = {
    serialized_data: [],
    loading: true
  };
  ...
  async componentDidMount() {
    try {
      const res = await fetch('http://127.0.0.1:8080/wms/api/expired_bin_inventory_type/');
      const expired_bin_data = await res.json();
      this.setState({
        serialized_data: expired_bin_data["data"],
        loading: false
      });
    } catch (e) {
      console.log(e);
...
export default withRouter(Expired_Bin_Inventory);
